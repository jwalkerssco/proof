...
t("findHeaderRow recognises VIP's Comparison header", () => {
  // Four KNOWN names on the row. Without them the header is missed, row 0 is
  // read as data, and "Brands" imports as a product called Brands.
  const rows = [["Brands", "Item Names", "Item Name ID", "Product Classes", ""],
                ["Michelob Ultra", "Ultra 1x30 12oz Can", "18030", "Beer", ""]];
  assert.strictEqual(findHeaderRow(rows), 0);
});
